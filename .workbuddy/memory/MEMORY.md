...
## 技术栈与结构
- 前端：Vue 3.5 + TS strict + Vite 6 + Pinia + Vue Router(hash) + CSS Design Tokens（tokens.css，禁 hex 硬编码）；5 个 service 全 Mock 化，UI 与数据层解耦。
...
- 沙箱内无法对本地端口实机 HTTP 冒烟（透明代理 AUTH_REQUIRED，SERVER__PORT 被注入覆盖）→ 以 MockMvc 集成测试为验收基准。
