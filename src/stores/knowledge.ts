...
export const useKnowledgeStore = defineStore('knowledge', () => {
  ...
  async function reloadAll(): Promise<void> {
    ...
    // 只保留新树中真实存在的展开项；全部失效（如切换 Vault）则用默认展开
    const folderIds = new Set(collectFolderIds(tree.value));
    const valid = [...keepExpanded].filter((id) => folderIds.has(id));
    expandedFolders.value = valid.length ? new Set(valid) : new Set();
    if (!valid.length) applyDefaultExpansion();
